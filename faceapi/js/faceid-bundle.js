(function () {
  ...
  const HTML = `
    <div id="fidStage" style="position:relative;width:280px;height:280px;flex-shrink:0;">
      ...
      <svg id="fidSvg" viewBox="-140 -140 280 280" xmlns="http://www.w3.org/2000/svg"
        ...
        <circle id="fidCalibRing" cx="280" cy="0" r="108" fill="none" stroke="#378ADD" stroke-width="2.5"
  ...
  // const MP_CDN_BASE = `assets/vendor/libs/faceapi/js`;
  // const CAM_UTILS   = `assets/vendor/libs/faceapi/js/camera_utils.js`;
  // const FACE_MESH   = `assets/vendor/libs/faceapi/js/face_mesh.js`;
  // const FACEAPI_CDN = "assets/vendor/libs/faceapi/js/face-api.min.js";
  // const FACEAPI_MODELS = "assets/vendor/libs/faceapi/weights";
  const MP_CDN_BASE = (window.LIB_URL && window.LIB_URL.faceApi) || d.redirect || `assets/vendor/libs/faceapi/js`;
  const CAM_UTILS   = (window.LIB_URL && window.LIB_URL.cameraUtils) || d.redirect || `assets/vendor/libs/faceapi/js/camera_utils.js`;
  const FACE_MESH   = (window.LIB_URL && window.LIB_URL.faceMesh) || d.redirect || `assets/vendor/libs/faceapi/js/face_mesh.js`;
  const FACEAPI_CDN = (window.LIB_URL && window.LIB_URL.faceApiMin) || d.redirect || "assets/vendor/libs/faceapi/js/face-api.min.js";
  const FACEAPI_MODELS = (window.LIB_URL && window.LIB_URL.faceApiModels) || d.redirect || "assets/vendor/libs/faceapi/weights";
  ...
  const baseUrl = window.location.origin + '/' + pathArray[1] + '/' + pathArray[2] + '/';

  const C = Object.assign({
    loginUrl           : baseUrl + "/auth/process_login",
  ...
  async function submitLogin() {
    ...
    try {
      const res  = await fetch(C.loginUrl, { method:"POST", headers:{"Content-Type":"application/json","X-Requested-With":"XMLHttpRequest"}, body: JSON.stringify(payload) });
